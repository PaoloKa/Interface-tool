...
public final class Index {
	...
	public boolean packIndex(int id, Store originalStore, boolean checkCRC) {
		try {
			...
			for (int archiveId : originalIndex.table.getValidArchiveIds()) {
				if (checkCRC && archiveExists(archiveId) && originalIndex.table.getArchives()[archiveId]
						.getCRC() == table.getArchives()[archiveId].getCRC()) {
					continue;
				}
	...
	public boolean putArchive(int id, int archiveId, int targetArchiveId, Store originalStore, boolean rewriteTable,
			boolean resetCache) {
		try {
			Index originalIndex = originalStore.getIndexes()[id];
			byte[] data = originalIndex.getMainFile().getArchiveData(archiveId);
			if (data == null)
				return false;
			if (!archiveExists(targetArchiveId))
				table.addEmptyArchiveReference(targetArchiveId);
			ArchiveReference reference = table.getArchives()[targetArchiveId];
			reference.updateRevision();
			ArchiveReference originalReference = originalIndex.table.getArchives()[archiveId];
			reference.copyHeader(originalReference);
			int revision = reference.getRevision();
			data[data.length - 2] = (byte) (revision >> 8);
			data[data.length - 1] = (byte) revision;
			if (!mainFile.putArchiveData(targetArchiveId, data))
				return false;
			if (rewriteTable && !rewriteTable())
				return false;
			if (resetCache)
