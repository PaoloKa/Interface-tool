	...
	public byte[] getArchiveData(int archiveId) {
		synchronized (data) {
			try {
				...
				index.read(tempBuffer, (long) ((long) archiveId * (long) IDX_BLOCK_LEN));
				tempBuffer.flip();
	...
	public boolean putArchiveData(int archiveId, ByteBuffer archive, int size, boolean exists) {
		synchronized (data) {
			try {
				long block;
				if (exists) {
					if ((long) ((long) archiveId * (long) IDX_BLOCK_LEN) + (long) IDX_BLOCK_LEN > (long) index.size()) {
						return false;
					}
					...
					block = (long) (data.size() + TOTAL_BLOCK_LEN - 1) / TOTAL_BLOCK_LEN;
					if (block == 0) {
						block = 1;
					}
				}
				...
				tempBuffer.flip();
				index.write(tempBuffer, (long) ((long) archiveId * (long) IDX_BLOCK_LEN));
				...
				while (remaining > 0) {
					...
					if (exists) {
						...
						data.read(tempBuffer, (long) ((long) block * (long) TOTAL_BLOCK_LEN));
						tempBuffer.flip();
					...
					if (nextBlock == 0) {
						exists = false;
						nextBlock = (long) ((data.size() + TOTAL_BLOCK_LEN - 1) / TOTAL_BLOCK_LEN);
						if (nextBlock == 0) {
							nextBlock = 1;
						}
						if (nextBlock == block) {
					...
					data.write(tempBuffer, (long) ((long) block * (long) TOTAL_BLOCK_LEN));
					remaining -= blockSize;
					block = nextBlock;
					chunk++;
				}
