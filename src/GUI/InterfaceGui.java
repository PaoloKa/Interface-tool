...
public class InterfaceGui  extends JFrame {
	...
	public InterfaceGui() {
		...
		textField_2.setBounds(11, 127, 86, 20);
		generalTab.add(textField_2);
		...
		chckbxRefreshTreeOn = new JCheckBox("refresh tree on save");
	 ...
	 * @return
	 */
	public static BufferedImage createHorizontalFlipped(BufferedImage img) {
        int w = img.getWidth();
        int h = img.getHeight();
	 ...
	 * @return
	 */
	private  BufferedImage createVerticalFlipped(BufferedImage img) {
			int w = img.getWidth();
	        int h = img.getHeight();
			   ...
			   */
	        	...
	        	int y  = ComponentDefinition.getY(component, interfaceId);
			  	...
			  	if(parent != null) {
			        ...
			        */
				 if(component.type == 5 && component.spriteId > -1){
			         try {
					 BufferedImage sprite = resize(ImageIO.read(new File(PropertyValues.sprite_path+component.spriteId+"_0.png")),width,height);
					 /* horizontal flip*/
					 if(component.hFlip)
						 sprite = createHorizontalFlipped(sprite);
					 /* vertical flip*/
					 if(component.vFlip)
						 sprite =  createVerticalFlipped(sprite);
					 g.drawImage(sprite, x, y, null);
			         } catch(Exception ex) {
			        	// JOptionPane.showMessageDialog(scrollPane_2, "Sprite "+component.spriteId+" could not be found, make sure that you've it in your dump folder.");
			        	 continue;
			         }
		         ...
		         * Containers
		         */
		        if(component.type == 0 && showContainers){
	 ...
	 * @return
	 */
	  private static BufferedImage colorImage(BufferedImage image, Color col) {
